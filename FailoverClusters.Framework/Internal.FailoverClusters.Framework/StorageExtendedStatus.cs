...
public class MSFT_StorageExtendedStatus : CIM_Error
{
	...
	{
		foreach (CimSubscriptionResult item in session.Subscribe("root/microsoft/windows/storage", "WQL", "SELECT * FROM MSFT_StorageExtendedStatus"))
		{
	...
	public static IDisposable Subscribe(CimSession session, IObserver<MSFT_StorageExtendedStatus> observer)
	{
		...
		return session.SubscribeAsync("root/microsoft/windows/storage", "WQL", "SELECT * FROM MSFT_StorageExtendedStatus").Subscribe(observer2);
	}
}
