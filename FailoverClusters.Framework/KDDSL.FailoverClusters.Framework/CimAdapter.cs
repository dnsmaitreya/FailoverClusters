...
using FailoverClusters.UI.Common.Reactive;
...
internal class CimAdapter : RootAdapterBase, IConnectionAdapter
{
	internal class AdapterBase
	{
		...
		public CimInstance[] ParseCimPrivateProperties(ClusterPropertyCollection propertyCollection)
		{
			...
			propertyCollection.Where((ClusterProperty property) => property.PropertyKind == ClusterPropertyKind.Private && property.IsModified).ForEach(delegate(ClusterProperty modifiedProperty)
			{
				...
				{
					cimInstance = new CimInstance("MSFTCluster_BinaryProperty", "root\\windows\\cluster");
				}
				else if (modifiedProperty is ClusterPropertyDateTime)
				{
					cimInstance = new CimInstance("MSFTCluster_FiletimeProperty", "root\\windows\\cluster");
				}
				else if (modifiedProperty is ClusterPropertyExpandString)
				{
					cimInstance = new CimInstance("MSFTCluster_ExpandStringProperty", "root\\windows\\cluster");
				}
				else if (modifiedProperty is ClusterPropertyExpandedString)
				{
					cimInstance = new CimInstance("MSFTCluster_ExpandedStringProperty", "root\\windows\\cluster");
				}
				else if (modifiedProperty is ClusterPropertyUShort)
				{
					cimInstance = new CimInstance("MSFTCluster_UInt16Property", "root\\windows\\cluster");
				}
				else if (modifiedProperty is ClusterPropertyInt)
				{
					cimInstance = new CimInstance("MSFTCluster_Int32Property", "root\\windows\\cluster");
				}
				else if (modifiedProperty is ClusterPropertyUInt)
				{
					cimInstance = new CimInstance("MSFTCluster_UInt32Property", "root\\windows\\cluster");
				}
				else if (modifiedProperty is ClusterPropertyLong)
				{
					cimInstance = new CimInstance("MSFTCluster_Int64Property", "root\\windows\\cluster");
				}
				else if (modifiedProperty is ClusterPropertyULong)
				{
					cimInstance = new CimInstance("MSFTCluster_UInt64Property", "root\\windows\\cluster");
				}
				else if (modifiedProperty is ClusterPropertyMultipleStrings)
				{
					cimInstance = new CimInstance("MSFTCluster_StringArrayProperty", "root\\windows\\cluster");
				}
				else if (modifiedProperty is ClusterPropertyString)
				{
					cimInstance = new CimInstance("MSFTCluster_StringProperty", "root\\windows\\cluster");
				}
				if (cimInstance != null)
				{
		...
		public void InvokeMethod(CimClass cimClass, string methodName, IEnumerable<CimArgumentNameValue> parameters, Action<CimMethodResult> action, string cimNamespace = "root\\windows\\cluster")
		{
		...
		public void InvokeMethod(CimClass cimClass, string methodName, IEnumerable<CimArgumentNameValue> parameters, Action<CimMethodResult> action, CimSession session, string cimNamespace = "root\\windows\\cluster", TimeSpan? timeout = null)
		{
			Func<CimMethodResult, object> action2 = delegate(CimMethodResult instance)
			{
				action.SafeCall(instance);
	...
	internal class CimConstants
	{
		...
		public const string Namespace = "root\\windows\\cluster";
	...
	private class StorageAdapter : AdapterBase, IConnectionAdapterStorage
	{
		private class StorageEnclosureToPhysicallyConnectedStorageNode
		{
			...
			public IEnumerable<MSFT_StorageNode> StorageNode
			{
				get
				{
					...
					if (Session.EnumerateAssociatedInstances("root/windows/storage", Instance, "MSFT_StorageNodeToStorageEnclosure", "MSFT_StorageNode", "StorageEnclosure", "StorageNode", cimOperationOptions) != null)
					{
						return (from i in Session.EnumerateAssociatedInstances("root/windows/storage", Instance, "MSFT_StorageNodeToStorageEnclosure", "MSFT_StorageNode", "StorageEnclosure", "StorageNode", cimOperationOptions)
		...
		private const string ReplicaNameSpace = "root\\windows\\StorageReplica";
		...
		private void SetReplicationGroupLogSize(CimSession session, string computerName, string replicationGroup, long logSize)
		{
			...
			}, session, "root\\windows\\StorageReplica", TimeSpan.FromMinutes(5.0));
		}
						...
						GetReplicationGroup(remoteSession, storageResource, delegate(CimInstance replicationGroupInstance)
						{
							...
							if (num)
							{
								...
								GetReplicationPartnership(remoteSession, replicationGroupInstance, ReplicationGroupRole.Primary, delegate(CimInstance cimPartnership)
								{
									...
									}, localSession, "root\\windows\\StorageReplica", TimeSpan.FromMinutes(5.0));
								});
							}
							else
							{
		...
		private IEnumerable<CimInstance> ExecuteReplicationQuery(CimSession session, string query)
		{
			...
			ClusterLog.LogInfo("Executing Query in replication namespace to execute query {{0}}", "root\\windows\\StorageReplica", query);
			using TraceWmiQuery queryTracer = ClusterLog.StartTraceQuery(FailoverClusters.UI.Common.TraceSource.Framework, query);
			int instances = 0;
			using (CimOperationOptions options = new CimOperationOptions())
			{
				options.Timeout = TimeSpan.FromSeconds(60.0);
				foreach (CimInstance item in session.QueryInstances("root\\windows\\StorageReplica", "WQL", query, options))
				{
					instances++;
					yield return item;
				}
		...
		{
			return session.GetClass("root\\windows\\StorageReplica", "MSFT_WvrAdminTasks");
		}
	...
	public void GetFirst(CimClass cimClass, Action<CimInstance> action)
	{
		Exceptions.ThrowIfNull(cimClass, "cimClass");
		Exceptions.ThrowIfNull(action, "action");
		using TraceWmiEnumeration traceWmiEnumeration = ClusterLog.StartTraceEnumeration(FailoverClusters.UI.Common.TraceSource.Framework, cimClass.CimSystemProperties.ClassName);
	...
	{
		return Scope.EnumerateAssociatedInstances("root\\windows\\cluster", sourceInstance, associationClassName, resultClassName, (associationType == AssociationType.CimDependency) ? "Antecedent" : "GroupComponent", (associationType == AssociationType.CimDependency) ? "Dependent" : "PartComponent");
	}
	...
	{
		return Scope.EnumerateReferencingInstances("root\\windows\\cluster", sourceInstance, associationClassName, (associationType == AssociationType.CimDependency) ? "Antecedent" : "GroupComponent");
	}

	public IEnumerable<CimInstance> ExecuteOptimalQuery(string query, bool nullElementOnError = true, string cimNamespace = "root\\windows\\cluster")
	{
		if (QueriesAreAsync)
		{
	...
	public IEnumerable<CimInstance> ExecuteQuery(string query, bool nullElementOnError = true, string cimNamespace = "root\\windows\\cluster")
	{
		Exceptions.ThrowIfNull(query, "query");
		ClusterLog.LogInfo("Executing Query in namespace {0} going to execute query {{1}}", "root\\windows\\cluster", query);
		using TraceWmiQuery queryTracer = ClusterLog.StartTraceQuery(FailoverClusters.UI.Common.TraceSource.Framework, query);
		int instances = 0;
		using (CimOperationOptions options = new CimOperationOptions())
		{
	...
	public IEnumerable<CimInstance> ExecuteQueryAsync(string query, bool nullElementOnError = true, string cimNamespace = "root\\windows\\cluster")
	{
		Exceptions.ThrowIfNull(query, "query");
		ClusterLog.LogInfo("Executing Query in namespace {0} going to execute query {{1}}", "root\\windows\\cluster", query);
		using TraceWmiQuery queryTracer = ClusterLog.StartTraceQuery(FailoverClusters.UI.Common.TraceSource.Framework, query);
	...
	{
		using (ClusterLog.StartTraceInvokeMethod(FailoverClusters.UI.Common.TraceSource.Framework, isStaticInvoke: false, Scope.ComputerName, "TestConnection", null))
		{
	...
	private void CreateSession(string clusterName, CimSessionOptions options)
	{
		...
		using (ClusterLog.StartTraceInvokeMethod(FailoverClusters.UI.Common.TraceSource.Framework, isStaticInvoke: false, relativePath, "CreateSession", null))
		{
	...
	private CimInstance ModifyInstance(CimInstance cimInstance)
	{
		...
		using (ClusterLog.StartTraceModifyInstance(FailoverClusters.UI.Common.TraceSource.Framework, cimInstance.GetRelativePath(), propertyName))
		{
	...
	private T GetInstance<T>(CimInstance identityInstance, Func<CimInstance, T> action)
	{
		...
		using (ClusterLog.StartTraceGetInstance(FailoverClusters.UI.Common.TraceSource.Framework, identityInstance.GetRelativePath()))
		{
	...
	private T InvokeMethod<T>(CimSession session, CimInstance cimInstance, CimClass cimClass, string methodName, IEnumerable<CimArgumentNameValue> parameters, Func<CimMethodResult, T> action, string cimNamespace = "root\\windows\\cluster", TimeSpan? timeout = null)
	{
		...
		return CommonAdapter.ExecuteAndCatchWmiExceptions(delegate
		{
			...
			using TraceWmiInvokeMethod traceWmiInvokeMethod = ClusterLog.StartTraceInvokeMethod(FailoverClusters.UI.Common.TraceSource.Framework, cimClass != null, relativePath, methodName, parameterNames);
			if (cimInstance != null)
			{
