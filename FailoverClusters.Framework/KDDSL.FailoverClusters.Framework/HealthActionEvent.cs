...
public class MSFT_HealthActionEvent : MSFT_StorageEvent
{
	...
	{
		foreach (CimSubscriptionResult item in session.Subscribe("root/windows/storage", "WQL", "SELECT * FROM MSFT_HealthActionEvent"))
		{
	...
	public static IDisposable Subscribe(CimSession session, IObserver<MSFT_HealthActionEvent> observer)
	{
		...
		return session.SubscribeAsync("root/windows/storage", "WQL", "SELECT * FROM MSFT_HealthActionEvent").Subscribe(observer2);
	}
}
