...
internal class NfsAdapter : FileShareDataSourceBase
{
	...
	private const string RawNfsNamespace = "root\\windows\\Nfs";
	...
	public override IObservable<IFileShareDataItem> GetSharesObservable(string nodeFqdn, string scopeName)
	{
		try
		{
			...
			CimObservableErrorType cimObservableErrorType = ServiceContainer.Container.Resolve<ICimUtilities>(Array.Empty<object>()).CanConnectToCim(cimSession, "root\\windows\\Nfs", "MSFT_NfsShare");
			...
			return ((IObservable<IFileShareDataItem>)(from result in cimSession.QueryInstancesAsync("root\\windows\\Nfs", "WQL", "select Name, Path, NetworkName, IsClustered from msft_NfsShare where networkname='{0}'".FormatCurrentCulture(scopeName), new CimOperationOptions
				{
	...
	{
		return GetShareEventObservable(nodeFqdn, "root\\windows\\Nfs", "MSFT_NfsShare", "SELECT * FROM MSFT_NfsShareEvent WHERE Share.NetworkName='{0}'".FormatInvariantCulture(serverName), "Share", "Event", TransformCimInstance);
	}
	...
	protected override CimInstance GetCimInstance(CimSession session, string serverName, string shareName, string sharePath)
	{
		...
		return FileShareDataSourceBase.GetCimInstance(session, "MSFT_NfsShare", "root\\windows\\Nfs", list);
	}
}
