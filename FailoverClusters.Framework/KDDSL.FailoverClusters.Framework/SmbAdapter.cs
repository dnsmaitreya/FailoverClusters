...
using FailoverClusters.Framework.CustomExceptions;
...
{
	private const string RawSmbNamespace = "root\\windows\\smb";
	...
	public override IObservable<IFileShareDataItem> GetSharesObservable(string nodeFqdn, string scopeName)
	{
		try
		{
			...
			CimObservableErrorType cimObservableErrorType = ServiceContainer.Container.Resolve<ICimUtilities>(Array.Empty<object>()).CanConnectToCim(cimSession, "root\\windows\\smb", "MSFT_SmbShare");
			...
			return ((IObservable<IFileShareDataItem>)(from result in cimSession.InvokeMethodAsync("root\\windows\\smb", "MSFT_SmbShare", "EnumerateShares", methodParameters, new CimOperationOptions
				{
	...
	{
		return GetShareEventObservable(nodeFqdn, "root\\windows\\smb", "MSFT_SmbShare", "SELECT * FROM MSFT_SmbShareChangeEvent WHERE Share.ScopeName='{0}'".FormatInvariantCulture(serverName), "Share", "EventType", TransformCimInstance);
	}
	...
	public void GrantAccess(string share, string account, SmbAccessRight smbAccess, FileSystemRights fsAccess)
	{
		...
		try
		{
			IEnumerable<CimInstance> cimShares = cimSession.QueryInstances("root\\windows\\smb", "WQL", "select * from Msft_SmbShare where Name='{0}'".FormatInvariantCulture(text2));
	...
	protected override CimInstance GetCimInstance(CimSession session, string scopeName, string shareName, string sharePath)
	{
		...
		return FileShareDataSourceBase.SearchCimInstances(session, "MSFT_SmbShare", "root\\windows\\smb", query).FirstOrDefault();
	}
}
