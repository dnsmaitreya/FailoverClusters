...
public class MSFT_SoftError : CIM_Error
{
	...
	{
		foreach (CimSubscriptionResult item in session.Subscribe("root/windows/storage", "WQL", "SELECT * FROM MSFT_SoftError"))
		{
	...
	public static IDisposable Subscribe(CimSession session, IObserver<MSFT_SoftError> observer)
	{
		...
		return session.SubscribeAsync("root/windows/storage", "WQL", "SELECT * FROM MSFT_SoftError").Subscribe(observer2);
	}
}
