...
public class MSFT_SoftError_NotAuthenticated : MSFT_SoftError
{
	...
	{
		foreach (CimSubscriptionResult item in session.Subscribe("root/windows/storage", "WQL", "SELECT * FROM MSFT_SoftError_NotAuthenticated"))
		{
	...
	public static IDisposable Subscribe(CimSession session, IObserver<MSFT_SoftError_NotAuthenticated> observer)
	{
		...
		return session.SubscribeAsync("root/windows/storage", "WQL", "SELECT * FROM MSFT_SoftError_NotAuthenticated").Subscribe(observer2);
	}
}
