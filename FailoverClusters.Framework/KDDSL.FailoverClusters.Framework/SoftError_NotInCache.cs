...
public class MSFT_SoftError_NotInCache : MSFT_SoftError
{
	...
	{
		foreach (CimSubscriptionResult item in session.Subscribe("root/windows/storage", "WQL", "SELECT * FROM MSFT_SoftError_NotInCache"))
		{
	...
	public static IDisposable Subscribe(CimSession session, IObserver<MSFT_SoftError_NotInCache> observer)
	{
		...
		return session.SubscribeAsync("root/windows/storage", "WQL", "SELECT * FROM MSFT_SoftError_NotInCache").Subscribe(observer2);
	}
}
