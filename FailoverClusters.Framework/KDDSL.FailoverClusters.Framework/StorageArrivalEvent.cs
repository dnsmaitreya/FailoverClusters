...
public class MSFT_StorageArrivalEvent : MSFT_StorageEvent
{
	...
	{
		foreach (CimSubscriptionResult item in session.Subscribe("root/windows/storage", "WQL", "SELECT * FROM MSFT_StorageArrivalEvent"))
		{
	...
	public static IDisposable Subscribe(CimSession session, IObserver<MSFT_StorageArrivalEvent> observer)
	{
		...
		return session.SubscribeAsync("root/windows/storage", "WQL", "SELECT * FROM MSFT_StorageArrivalEvent").Subscribe(observer2);
	}
}
