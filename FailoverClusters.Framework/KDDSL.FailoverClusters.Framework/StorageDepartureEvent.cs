...
public class MSFT_StorageDepartureEvent : MSFT_StorageEvent
{
	...
	{
		foreach (CimSubscriptionResult item in session.Subscribe("root/windows/storage", "WQL", "SELECT * FROM MSFT_StorageDepartureEvent"))
		{
	...
	public static IDisposable Subscribe(CimSession session, IObserver<MSFT_StorageDepartureEvent> observer)
	{
		...
		return session.SubscribeAsync("root/windows/storage", "WQL", "SELECT * FROM MSFT_StorageDepartureEvent").Subscribe(observer2);
	}
}
