...
public class MSFT_StorageEvent : MiBase
{
	...
	{
		foreach (CimSubscriptionResult item in session.Subscribe("root/windows/storage", "WQL", "SELECT * FROM MSFT_StorageEvent"))
		{
	...
	public static IDisposable Subscribe(CimSession session, IObserver<MSFT_StorageEvent> observer)
	{
		...
		return session.SubscribeAsync("root/windows/storage", "WQL", "SELECT * FROM MSFT_StorageEvent").Subscribe(observer2);
	}
}
