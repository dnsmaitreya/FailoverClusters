...
public class MSFT_StorageHealthStatusChangeEvent : MSFT_StorageEvent
{
	...
	{
		foreach (CimSubscriptionResult item in session.Subscribe("root/windows/storage", "WQL", "SELECT * FROM MSFT_StorageHealthStatusChangeEvent"))
		{
	...
	public static IDisposable Subscribe(CimSession session, IObserver<MSFT_StorageHealthStatusChangeEvent> observer)
	{
		...
		return session.SubscribeAsync("root/windows/storage", "WQL", "SELECT * FROM MSFT_StorageHealthStatusChangeEvent").Subscribe(observer2);
	}
}
