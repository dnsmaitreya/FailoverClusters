...
public class MSFT_Synchronized : MiInstanceBase
{
	...
	{
		CimInstance instanceId = new CimInstance("MSFT_Synchronized", "root/windows/storage");
	...
	{
		CimInstance instance = new CimInstance("MSFT_Synchronized", "root/windows/storage");
		return new MSFT_Synchronized(session, instance);
	}

	public static IEnumerable<MSFT_Synchronized> Enumerate(CimSession session)
	{
		return from i in session.EnumerateInstances("root/windows/storage", "MSFT_Synchronized")
