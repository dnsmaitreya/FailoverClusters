...
public class MSFT_SoftError_RelativePath : MSFT_SoftError
{
	...
	{
		foreach (CimSubscriptionResult item in session.Subscribe("root/microsoft/windows/storage", "WQL", "SELECT * FROM MSFT_SoftError_RelativePath"))
		{
	...
	public static IDisposable Subscribe(CimSession session, IObserver<MSFT_SoftError_RelativePath> observer)
	{
		...
		return session.SubscribeAsync("root/microsoft/windows/storage", "WQL", "SELECT * FROM MSFT_SoftError_RelativePath").Subscribe(observer2);
	}
}
