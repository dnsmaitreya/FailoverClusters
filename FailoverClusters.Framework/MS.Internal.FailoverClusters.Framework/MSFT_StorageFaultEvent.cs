...
public class MSFT_StorageFaultEvent : MSFT_StorageEvent
{
	...
	{
		foreach (CimSubscriptionResult item in session.Subscribe("root/microsoft/windows/storage", "WQL", "SELECT * FROM MSFT_StorageFaultEvent"))
		{
	...
	public static IDisposable Subscribe(CimSession session, IObserver<MSFT_StorageFaultEvent> observer)
	{
		...
		return session.SubscribeAsync("root/microsoft/windows/storage", "WQL", "SELECT * FROM MSFT_StorageFaultEvent").Subscribe(observer2);
	}
}
