...
internal class PNetNameResource : PResource
{
	...
	public void UpdateShares(UpdateShareOperation updateOperation)
	{
		...
		Worker.Start(delegate
		{
			...
			do
			{
				...
				if (!string.IsNullOrWhiteSpace(netbiosName) && base.ResourceState == FailoverClusters.Framework.ResourceState.Online)
				{
					string nodeFqdn;
	...
	{
		if (e.State != FailoverClusters.Framework.ResourceState.Online && shares != null)
		{
			shares.Clear();
		}
	}
