...
internal class FilterGrid : DataGridView
{
	...
	private FilterGrouping m_filterGrouping;

	private int m_deletedRow;

	private bool m_painting;

	private string m_oldCellValue;

	private Color m_groupedLineColor;

	private Color m_groupedBackColor;

	private Color m_gridForeColor;

	private Color m_gridBackColor;

	private Color m_borderColor;

	private bool m_isAddingColumns;

	private bool m_isDeleting;

	private bool m_activated;

	private Dictionary<string, ClusterResource> resources;
	...
	public FilterGrid()
	{
		...
		InitializeComponent();
		InitializeGrid();
		resources = new Dictionary<string, ClusterResource>(StringComparer.OrdinalIgnoreCase);
	...
	public void DeleteSelectedClause()
	{
		...
		try
		{
			...
			for (int num = rowsSelectedForDeletion.Count - 1; num >= 0; num--)
			{
				...
				{
					m_filterGrouping.RemoveRow(rowsSelectedForDeletion[num].Index);
			...
			for (int i = 1; i < base.Rows.Count - 1; i++)
			{
				...
				{
					Connection connection = m_filterGrouping.GetConnection(i);
	...
	private void InitializeGrid()
	{
		...
		m_filterGrouping = new FilterGrouping();
		try
		{
			m_isAddingColumns = true;
		...
		{
			m_isAddingColumns = false;
		}
		m_borderColor = SystemColors.ControlDark;
		m_groupedLineColor = SystemColors.GrayText;
		m_groupedBackColor = SystemColors.Window;
		m_gridForeColor = SystemColors.ButtonFace;
		m_gridBackColor = SystemColors.Window;
		base.ColumnHeadersDefaultCellStyle.BackColor = m_gridForeColor;
		base.ColumnHeadersDefaultCellStyle.Font = null;
		base.RowHeadersDefaultCellStyle.BackColor = m_gridForeColor;
		base.BackgroundColor = m_gridBackColor;
		base.GridColor = m_gridForeColor;
		DataGridViewCellStyle dataGridViewCellStyle = new DataGridViewCellStyle(base.Columns[0].DefaultCellStyle);
		dataGridViewCellStyle.BackColor = m_gridForeColor;
	...
	protected override void OnPaint(PaintEventArgs e)
	{
		...
		e.Graphics.DrawRectangle(new Pen(new SolidBrush(m_borderColor)), rect);
	}
	...
	private void OnCellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
	{
		if (e.ColumnIndex > 0)
		{
			...
			if (m_filterGrouping.RowIsInGroup(e.RowIndex))
			{
				e.CellStyle.BackColor = m_groupedBackColor;
			}
		}
	}
	...
	private void PaintUncommittedRow(DataGridViewCellPaintingEventArgs e)
	{
		...
		e.Graphics.FillRectangle(new SolidBrush(e.CellStyle.BackColor), rectangle);
		StringFormat stringFormat = new StringFormat();
		stringFormat.Alignment = StringAlignment.Near;
		stringFormat.LineAlignment = StringAlignment.Center;
		stringFormat.Trimming = StringTrimming.None;
		stringFormat.FormatFlags |= StringFormatFlags.NoClip;
		e.Graphics.DrawString(filterGridClickHereToAddClause, e.CellStyle.Font, new SolidBrush(m_groupedLineColor), rectangle, stringFormat);
		Pen pen = new Pen(base.GridColor);
	...
	private void PaintGroupingCell(DataGridViewCellPaintingEventArgs e)
	{
		...
		{
			e.Handled = true;
			Pen pen = new Pen(m_groupedLineColor, 2f);
			Brush brush = new SolidBrush(e.CellStyle.BackColor);
			Brush b = new SolidBrush(m_groupedBackColor);
			e.Graphics.FillRectangle(brush, e.CellBounds);
			int num = 6;
			Rectangle r = new Rectangle(e.CellBounds.Right - num, e.CellBounds.Top, num, e.CellBounds.Height);
			PaintGroupingBox(e.Graphics, b, pen, r, m_filterGrouping.GetConnection(e.RowIndex));
		}
	}
	...
	protected override void OnUserAddedRow(DataGridViewRowEventArgs e)
	{
		...
		m_filterGrouping.AddRow(e.Row.Index);
		OnFilterExpressionChanged(EventArgs.Empty);
	}
	...
	protected override void OnUserDeletedRow(DataGridViewRowEventArgs e)
	{
		...
		m_filterGrouping.RemoveRow(m_deletedRow);
		base.Rows[0].Cells[1].Value = null;
		OnFilterExpressionChanged(EventArgs.Empty);
	}
	...
	protected override void OnCellBeginEdit(DataGridViewCellCancelEventArgs e)
	{
		...
		m_oldCellValue = GetCellValueAsString(cell);
	}
	...
	protected override void OnCellEndEdit(DataGridViewCellEventArgs e)
	{
		...
		FilterGridComboBoxCell cell = (FilterGridComboBoxCell)base.Rows[e.RowIndex].Cells[e.ColumnIndex];
		if (m_oldCellValue != GetCellValueAsString(cell))
		{
			OnFilterExpressionChanged(EventArgs.Empty);
		}
	}
	...
	public void NotifyComboBoxSelectionChange(object selectedValue)
	{
		if (base.CurrentCell.ColumnIndex == 1)
		{
			...
			{
				if (!m_filterGrouping.AddRowToAdjacentGroup(base.CurrentCell.RowIndex))
				{
					m_filterGrouping.AddGrouping(base.CurrentCell.RowIndex - 1, base.CurrentCell.RowIndex);
				}
				Refresh();
			}
			else if (string.Compare(strA, AndOperator, StringComparison.OrdinalIgnoreCase) == 0 && m_filterGrouping.RemoveRowFromGroup(base.CurrentCell.RowIndex))
			{
				Refresh();
			}
		}
	...
	public void SetResources(List<ClusterResource> resourceList)
	{
		resources.Clear();
		displayNames.Clear();
		foreach (ClusterResource resource in resourceList)
		{
			string text = resource.DisplayName;
			int num = 1;
			while (resources.ContainsKey(text))
			{
				text = string.Format(CultureInfo.InvariantCulture, "{0} ({1})", resource.DisplayName, num);
				num++;
			}
			resources.Add(text, resource);
			displayNames.Add(resource.Name, text);
		}
	}

	public void SetDependencyExpression(ClusterResourceRelationship relationship)
	{
		try
		{
			base.Rows.Clear();
			m_filterGrouping.Clear();
			foreach (RelatedClusterResource childResource in relationship.ChildResources)
			{
				AddRow(relationship.RelationshipType, displayNames[childResource.DisplayName]);
			}
			if (relationship.RelationshipType == ResourceRelationshipType.Or && relationship.ChildResources.Count > 1)
			{
				m_filterGrouping.AddGrouping(0, relationship.ChildResources.Count - 1);
			}
			foreach (ClusterResourceRelationship childRelationship in relationship.ChildRelationships)
			{
				...
				{
					AddRow((i == 0) ? relationship.RelationshipType : childRelationship.RelationshipType, displayNames[childRelationship.ChildResources[i].DisplayName]);
				}
				m_filterGrouping.AddGrouping(num, num + childRelationship.ChildResources.Count - 1);
			}
			OnFilterExpressionChanged(EventArgs.Empty);
		}
		finally
	...
	private void AddRow(ResourceRelationshipType logicalOperator, string resource)
	{
		DataGridViewRow dataGridViewRow = new DataGridViewRow();
		dataGridViewRow.CreateCells(this);
		if (base.Rows.Count > 1)
		{
			dataGridViewRow.Cells[1].Value = ((logicalOperator == ResourceRelationshipType.And) ? AndOperator : OrOperator);
		}
	...
	public void InsertRow(int index)
	{
		DataGridViewRow dataGridViewRow = new DataGridViewRow();
		dataGridViewRow.CreateCells(this);
		m_filterGrouping.AddRow(index);
		if (index == 0)
		{
			base.Rows[0].Cells[1].Value = AndOperator;
		}
		else if (m_filterGrouping.RowIsInGroup(index))
		{
	...
	private string GetDependencyExpression(bool friendly)
	{
		...
		for (int i = 0; i < base.Rows.Count; i++)
		{
			...
			if (text.Length == 0 && i != 0)
			{
				if (!friendly)
				{
					throw ExceptionHelp.Build<ClusterInputValidationException>(new string[1] { Resources.MissingOperator_Text });
				}
			...
			if (text2.Length == 0)
			{
				if (!friendly)
				{
					throw ExceptionHelp.Build<ClusterInputValidationException>(new string[1] { Resources.MissingDependecy_Text });
				}
	...
	public ClusterResource[] GetDependencies()
	{
		CommitCurrentCell();
		ClusterResource[] array = new ClusterResource[base.Rows.Count - 1];
