...
internal class ResourcePoliciesPropertyPage : ResourcePropertyPage
{
	internal enum ClusterResourceRestartAction
	{
	...
	internal enum ResourceRestartAction
	{
		DoNotRestart,
		RestartNoNotify,
		RestartNotify
	...
	internal ResourcePoliciesPropertyPage()
	{
		InitializeComponent();
	}

	public ResourcePoliciesPropertyPage(Microsoft.FailoverClusters.Framework.Cluster cluster, Guid resourceId)
		: base(Resources.Policies_Text)
	{
		...
		Resource.Get(cluster, resourceId, delegate(OperationResult<Resource> cacheResult)
		{
			...
			resource.LoadAsync(delegate(ClusterLoadedEventArgs result)
			{
				...
				SnapinPropertyPageControlBase.UpdateControl((Control)(object)this, delegate
				{
					...
					restartAction = (ResourceRestartAction)(uint)properties["RestartAction"].Value;
					restartThreshold = (uint)properties["RestartThreshold"].Value;
					restartPeriod = (uint)properties["RestartPeriod"].Value;
					pendingTimeout = (uint)properties["PendingTimeout"].Value;
					retryPeriodOnFailure = (uint)properties["RetryPeriodOnFailure"].Value;
					restartDelay = (uint)properties["RestartDelay"].Value;
					LoadControls();
				});
			}, ResourceLoadSelection.CommonProperties);
		}, OperationType.Async);
		return null;
	}
	...
	protected override bool ValidateProperties()
	{
		...
		{
			restartAction = ((!failoverGroupCheckBox.Checked) ? ResourceRestartAction.RestartNoNotify : ResourceRestartAction.RestartNotify);
		}
	...
	protected override bool SaveProperties()
	{
		...
		{
			ClusterPropertyCollection properties = resource.Properties;
			((ClusterPropertyUInt)properties["RestartAction"]).TypedValue = (uint)restartAction;
			((ClusterPropertyUInt)properties["RestartThreshold"]).TypedValue = restartThreshold;
			((ClusterPropertyUInt)properties["RestartPeriod"]).TypedValue = restartPeriod;
			((ClusterPropertyUInt)properties["RetryPeriodOnFailure"]).TypedValue = retryPeriodOnFailure;
			((ClusterPropertyUInt)properties["PendingTimeout"]).TypedValue = pendingTimeout;
			((ClusterPropertyUInt)properties["RestartDelay"]).TypedValue = restartDelay;
			properties.Commit(delegate(OperationResult result)
			{
				...
				else
				{
					...
					if (!success)
					{
						...
						ClusterLog.LogException(ex, "Error saving resource policies properties");
	...
	private void InitializeComponent()
	{
		...
		ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(ResourcePoliciesPropertyPage));
		...
		((Control)(object)this).Name = "ResourcePoliciesPropertyPage";
