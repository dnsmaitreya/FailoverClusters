...
public abstract class Resource : ClusterObject
{
	...
	public ResourceSubStatus? ResourceSubStatus => LoadAsync(subStatus, 2, () => Microsoft.FailoverClusters.Framework.ResourceSubStatus.None);
	...
	private IEnumerable<string> ParseProperties(ClusterPropertyCollection properties, bool trackChanges)
	{
		...
		if (ParseProperty(properties, "LastOperationStatusCode", ref lastOperationStatusCode, list) && lastOperationStatusCode.HasValue)
		{
			...
			{
				if ((this is VirtualMachineResource || this is VirtualMachineConfigurationResource) && subStatus.HasValue && !subStatus.Value.HasFlag(Microsoft.FailoverClusters.Framework.ResourceSubStatus.Locked))
				{
